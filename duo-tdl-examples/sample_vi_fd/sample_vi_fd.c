...
static CVI_TDL_SUPPORTED_MODEL_E g_face_model_id = CVI_TDL_SUPPORTED_MODEL_SCRFDFACE;
...
void *run_tdl_thread(void *pHandle) {
  ...
  while (bExit == false) {
    ...
    memset(&stFaceMeta, 0, sizeof(cvtdl_face_t));
    s32Ret = CVI_TDL_FaceDetection(pstTDLHandle, &stFrame, g_face_model_id, &stFaceMeta);
    if (s32Ret != CVI_TDL_SUCCESS) {
...
int main(int argc, char *argv[]) {
  ...
  // Keep VB pools unbound on this target to avoid attach-related runtime crash.
  stMWConfig.stVBPoolConfig.astVBPoolSetup[0].bBind = false;
  ...
  stMWConfig.stVBPoolConfig.astVBPoolSetup[1].bBind = false;
  ...
  const char *pszFaceModel = getenv("SAMPLE_TDL_FACE_MODEL");
  if (pszFaceModel != NULL && strcmp(pszFaceModel, "retina") == 0) {
    g_face_model_id = CVI_TDL_SUPPORTED_MODEL_RETINAFACE;
  }
  printf("TDL face model id=%d (%s)\n", g_face_model_id, CVI_TDL_GetModelName(g_face_model_id));

  // Guard against model-enum/file mismatch that can abort inside closed-source TDL runtime.
  if (g_face_model_id == CVI_TDL_SUPPORTED_MODEL_RETINAFACE &&
      str_contains_case_insensitive(argv[1], "scrfd")) {
    printf("Model mismatch: selected RETINAFACE but file looks like SCRFD: %s\n", argv[1]);
    printf("Use a RetinaFace cvimodel, or unset SAMPLE_TDL_FACE_MODEL to use SCRFDFACE.\n");
    return -1;
  }

  // A3 (Workaround): Always use explicit VPSS group assignment to avoid handle binding issues.
  // Previously: auto handle in no-RTSP mode, explicit handle in RTSP mode.
  // Now: always explicit for consistency.
  ...
  // Extended timeout may help if model tensor setup takes longer than 1s.
  CVI_TDL_SetVpssTimeout(stTDLHandle, 3000);

  cvitdl_service_handle_t stServiceHandle = NULL;
  printf("TDL setup: Service_CreateHandle...\n");
  GOTO_IF_FAILED(CVI_TDL_Service_CreateHandle(&stServiceHandle, stTDLHandle), s32Ret,
                 create_service_fail);

      // Keep compatibility with older runtime libtdl builds that may not export
      // optional tuning APIs (e.g. CVI_TDL_UseMmap/CVI_TDL_SetVpssThread).
      printf("TDL setup: skip optional mmap/thread tuning for runtime compatibility\n");
  ...
  // Set confidence threshold to ensure model is in proper inference state.
  ...
  if (!bDisableRTSP) {
    ...
    printf("SAMPLE_TDL_DISABLE_RTSP=1, run TDL thread only\n");
  }
